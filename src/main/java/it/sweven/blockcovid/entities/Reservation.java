...
public class Reservation {
  ...
  @Transient private static final ChronoUnit granularity = ChronoUnit.MINUTES;
  ...
  public Reservation(
      String nameRoom, Integer idDesk, LocalDate date, LocalTime from, LocalTime to, String user) {
    this.nameRoom = nameRoom;
    this.idDesk = idDesk;
    if (date.isBefore(LocalDate.now())) throw new IllegalArgumentException("'date' already passed");
    this.date = date;
    if (from.isAfter(to)) throw new IllegalArgumentException("'from' should come before 'to'");
    this.from = from.truncatedTo(granularity);
    this.to = to.truncatedTo(granularity);
    this.user = user;
  }
  ...
  public void setTo(LocalTime to) {
    if (from.isAfter(to)) throw new IllegalArgumentException("'to' should come after 'from'");
    this.to = to.truncatedTo(granularity);
  }
  ...
  public boolean conflicts(Reservation other) {
    if (this.nameRoom.equals(other.nameRoom)
        && this.idDesk.equals(other.idDesk)
